...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const bioMessages = [
    ...
    "ashqryl ~ downtown where the lights are blinding"
  ];
  let bioText = '';
  let bioIndex = 0;
  ...
  const presenceActivity = document.getElementById('presence-activity');

  function updatePresence(data) {
    const d = data.data;
    // status dot
    presenceDot.className = 'status-dot ' + (d.discord_status || 'offline');
    // avatar
    if (d.discord_user) {
      const avatarHash = d.discord_user.avatar;
      if (avatarHash) {
        presenceAvatar.src = `https://cdn.discordapp.com/avatars/${DISCORD_ID}/${avatarHash}.png?size=64`;
        presenceAvatar.style.display = 'block';
      }
      presenceName.textContent = d.discord_user.global_name || d.discord_user.username;
    }
    // activity
    const act = d.activities && d.activities.find(a => a.type !== 4);
    const custom = d.activities && d.activities.find(a => a.type === 4);
    if (act) {
      presenceActivity.textContent = `${act.name}${act.details ? ' — ' + act.details : ''}`;
    } else if (custom && custom.state) {
      presenceActivity.textContent = custom.state;
    } else {
      const statusMap = { online: 'Online', idle: 'Away', dnd: 'Do Not Disturb', offline: 'Offline' };
      presenceActivity.textContent = statusMap[d.discord_status] || 'Offline';
    }
  }
  ...
  function connectLanyard() {
    ...
    ws.onmessage = (e) => {
      ...
      if (msg.op === 1) {
        ...
        updatePresence({ data: msg.d });
      } else if (msg.op === 0 && msg.t === 'PRESENCE_UPDATE') {
        updatePresence({ data: msg.d });
      }
    };
    ws.onclose = () => { clearInterval(heartbeat); setTimeout(connectLanyard, 3000); };
    ws.onerror = () => ws.close();
  }
  connectLanyard();
  // ─────────────────────────────────────────────────────────────────
